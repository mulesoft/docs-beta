...
|`exception`
a|`error`
   Exceptions within a flow are now represented by a
   xref:error-handling.adoc[Mule Error] that you access with the `error`
   variable.
...
|`message.inboundAttachments` (Inbound attachments)
a|Replaced by multipart data formats:
  See xref:migration-tool-post-mig.html#inbound_attachments[Inbound Attachments]
  and xref:intro-mule-message.adoc#attachments[Attachments].

a|`message.inboundProperties` (Inbound properties)
  Examples: `#[inboundProperties.'http.query.params']`, `#[inboundProperties]` (for HTTP headers)
a|`attributes` (Attributes for message metadata)
  Examples: `#[attributes.queryParams]`, `#[attributes.headers]` (for HTTP headers)
  See xref:intro-mule-message.adoc#inbound-properties-are-now-attributes[Inbound Properties Are Now Attributes].

|`message.outboundAttachments` (Outbound attachments)
a|Replaced by multipart data formats:
  See xref:migration-tool-post-mig.html#outbound_attachments[Outbound Attachments]
  and xref:intro-mule-message.adoc#attachments[Attachments].

|`message.outboundProperties` (Outbound properties)
a|Removed:
  See xref:intro-mule-message.adoc#outbound-properties[Outbound Properties].

a|`recordVars` (Record variables)
  Example: `#[recordVars.myVar]`
a|`var`

a|`sessionVars` (Session Variables)
  Example: `#[sessionVars.myVar]`
a|Removed:
  Transport barriers do not exist in Mule 4 (see
  xref:intro-mule-message.adoc#session-properties[Session Properties]).
