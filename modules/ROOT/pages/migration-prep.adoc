...
For projects that are already in development or deployed on Mule 3 (whether a prospect or customer), MuleSoft recommends that you wait to migrate these applications until the Mule Migration Assistant (MMA) is available. Using the MMA tool, you would speed up the heavy lifting work of re-creating the application from scratch in Mule 4.
...
First, you will need to download Studio 7 and install it. If you're deploying locally, you will also need to download the Mule 4 standalone runtime, if the one that comes with Studio does not match the one you want to use for runtime purposes.
// TODO link to pages
