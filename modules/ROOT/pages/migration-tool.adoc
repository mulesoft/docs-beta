...
* Modules: Validations, Scripting (DSL only, no migration of the script itself), JSON (only validations)
* Transports: HTTP, JMS, File, FTP, SFTP, VM, Email, Quartz
* Components: Cache, Batch, For Each, Choice, Scatter-Gather, Until-Successful, First-Successful, Async, Enricher, Poll, Watermark, Filters
* GZip transformers
* Error Handling, Transactional scope, Composite sources, Watermark
* Properties and secure properties
* OAuth Provider, Security filters, Checksum operations
...
The Mule Migration tool does not currently support the following components:
...
Connectors that are used in an app but not currently supported by the Mule Migration
