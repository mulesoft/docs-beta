...
These steps are detailed in xref:migration-tool-procedure.adoc[Perform a Migration to Mule 4]. However, before proceeding with a migration, it is important to review the rest of this topic, which describes the <<migration_tool>>, the <<compatibility_module>>, and <<whats_migrated>>.
