...
. Import the MMA Mule 4 converted application into a supported version of Studio 7.
. Review the MMA Migration report.
. Perform a manual migration on what MMA cannot migrate.
...
MMA helps with such heavy lifting, but it is not to be expected to
...
module as a bridge until the manual steps of the migration are complete and it is 100% compliant with Mule 4.
...
Connectors that are used in an app but not currently supported by MMA
