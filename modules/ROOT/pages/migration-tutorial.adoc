...
Note that for cases in other apps where you _do_ need to handle a `statusCode`
...
to specific documentation on performing this migration, in this case to  xref:migration-tool-post-mig.doc#outbound_properties[Outbound Properties (Manual Post-Migration Steps)].
...
NOTE: Before deploying the app, you might need to set the HTTP port to an
actual value to run it, as well as going through the Readme tasks to
pre-populate the Database.

Now, you can also create the same Querying a MySQL Database app for Mule 4
from Exchange and compare the result.
