...
+
This process loads the app to a project into Studio 6. By default, the name of
...
----
$ java -jar mule-migration-tool-runner-0.3.0.jar -muleVersion 4.1.3 -projectBasePath /Users/home/AnypointStudio/workspace-studio6/querying-a-mysql-database-1.4.0-mule-application-example -destinationProjectBasePath /Users/home/myproject/querying-a-mysql-database-mule4
----
+
For details, see xref:migration-tool-procedure.adoc#options[Command-line Options].
+
* To discover the path to the Studio 6 project, you can navigate to
*File* -> *Switch Workspace* -> *Other* in Studio. Then you append the name
of the project to that path.
...
////
The report identifies items that remain to complete the migration to Mule 4. The
warnings (WARN messages) in this report are associated with components in the
...
`/Users/me/Downloads/querying-a-mysql-database-mule4/report/summary.html`.
+
. Address warnings and errors in the report:
+
* <<warning_1>>
* <<warning_2>>
* <<warning_3>>
...
In Mule 3, connectors and transports that need to send additional data, such
as headers, must explicitly specify outbound properties. For example, with the
HTTP listener, you might want to specify the outgoing status code response or
headers.
...
This compatibility processor reads attributes of the Mule message and generates
a Mule 3.x equivalent of inbound properties as a variable. Each time an
Inbound property, you need to use the Attributes object from the new Mule
message. Because there are no inbound properties, you can remove this component.
You can replace any future reference to an Inbound property with the attributes
object.
...
To be able to test the migrated app, you need a MySQL server with the `employees` database that the app queries, and you need to make sure that your database and HTTP listener connections work.

. Import this script into your MySQL server: link:{attachmentsdir}/create.sample.db.sql[Sample MySQL Script]
+
...
One way to import the script is through the *Data Import/Restore* area of MySQLWorkbench, using the *Import from Self-Contained File* field to point to
the script.
+
...
+
. In a web browser or app such as Postman, go to URL (such as `http://localhost:8080/?lastname=Howard`) that uses your HTTP listener configuration, and view the JSON results:
+
...
Note that you can also create the same *Querying a MySQL Database* app for Mule 4
from Exchange and compare your results.
