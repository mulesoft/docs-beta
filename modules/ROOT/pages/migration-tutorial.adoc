...
////
If you do not have them already, you can download MMA and Anypoint Studio
...
You will unzip this file to a folder. All runs of MMA in this docs must be run from inside this folder.
////
...
* Maven installation: Maven version 3.3.9 or later (see <<maven_install>>) is for manual, post-migration steps.
...
Run MMA on the Mule 3 app, and import the migrated app into Studio 7. In
subsequent sections, you will perform the manual steps needed to complete
...
----
$ java -jar mule-migration-assistant-runner-1.0.0.jar -muleVersion 4.5.0 -projectBasePath /Users/me/AnypointStudio/workspace-studio6/querying-a-mysql-database-1.4.0-mule-application-example -destinationProjectBasePath /Users/me/Downloads/querying-a-mysql-database-mule4
----
+
...
Now you open and review the MMA-generated xref:migration-report.adoc[Mule Migration report],
...
+
//TODO: ?ADD RECOMMENDED LIBRARY OR ADD MAVEN DEPENDENCY OR EITHER?????
. To add the recommended JDBC driver for this database, click the *Configure*
button and use *Add Maven Dependency* (or *Add recommended library*) to replace the existing driver.
//TODO: IS THIS STEP NEEDED, AND IF SO, HOW TO ACCOMPLISH IT?
...
To complete manual migration tasks that remain to do on a migrated Mule 4 app, you need to configure a local installation of Maven.

[[step_maven]]
=== Configure a Connection to the `releases-ee` Repository

If your migrated Mule 4 app contains *Unknown* components in Studio 7, make sure that Maven is installed and configured properly. A proper configuration enables the Studio 7 project for your migrated app to find and connect to the `releases-ee` repository. Once Studio establishes that connection and Studio loads the repository settings, the components will have the proper names and appearance (shown in <<step_3>>).

image::mmt-migrated-app-mvn-config-issue.png[Imported App][Imported App with Maven Configuration Issue]

. Make sure that Maven installed. See <<maven_install>> for details.
. Provide your Studio 7 project with credentials to the `releases-ee` repository.
+
...
the *User Settings* window, add edit the *Server Id* `releases-ee` with your credentials to the repository, for example:
+
...
. Reopen the project.
+
After Studio processes the configuration, the compatibility modules replace the *Unknown* components:
+
image::mmt-migrated-app.png[Imported App]
+
