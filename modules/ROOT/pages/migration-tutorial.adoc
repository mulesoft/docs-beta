...
To avoid or fix a migrated Mule 4 app that displays *Unknown* components in Studio 7, make sure that Maven is installed and configured properly. A proper configuration enables the Studio 7 project for your migrated app to find and connect to the `releases-ee` repository. After Studio establishes the connection to the `releases-ee` repository and loads the appropriate resources, the components will display the proper names and appearance (as shown in <<step_3>>).
