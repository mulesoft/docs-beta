...
[[known_issues]]
=== Known Issues
The list of known issues for MMA is:

* Policies with policy YAML files with `.yml` extension are not detected.
* Policy YAML file is not autocompleted with mandatory fields.
* Migration report fails when migrating a `HttpConnectorRequestConfig` element.
...
* Migrated element `ee:transform` does not add schema location URI.
* DataWeave expression starting with underscore is not quoted and causes policy to fail.
