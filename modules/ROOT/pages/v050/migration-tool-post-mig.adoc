...
using MMA, incorporating the Compatibility module (see
...
not supported by the migrator to generate the equivalent DataWeave automatically.
...
The Mule Migration Assistant (MMA) does not migrate the schema and example files
you use when creating custom metadata types in Studio 6.

To reproduce the custom data types in your migrated Studio 7 project:

. Locate the schema or example files used by your Mule 3 app.
+
People often store them in the Studio 6 `src/main/resources` directory for the
project, though they might be stored other locations in your file system.
+
. Open the migrated Mule 4 app in Studio 7.
. Use the Transform Message component in your Studio 7 project to load the
custom data type files. You can use these steps as a general guide to this
process:
...
Studio window).
+
The columned input on the left supports custom metadata for the *Payload*,
*Variables*, or *Attributes*.
+
.. Determine whether to add your custom metadata type for the payload,
attributes, or variables.
+
https://docs.mulesoft.com/mule-runtime/4.1/dataweave-quickstart#med_large_payload[This example] (starting in step 4) loads the content of an example JSON file as a
...
+
For example, you might click *Define metadata* for  *Payload* or *Attributes*,
or you can simply right-click one of them, and then then click
*Set Metadata* to open the *Select metadata type* dialog.
+
For variables, you have to provide a variable name before the
*Select metadata type* dialog opens.
+
. Now, one by one, load your schema and example files using these steps for
guidance with each custom date type:
+
...
.. Below the new type, select *Schema* to *Example*, whichever applies to your
custom metadata type.
...
.. Repeat these steps for each Transform Message component in your migrated app.
...
because MMA does not have the full context to interpret the semantics of an
app.

[[session_variables]]
== Session Properties (Manual Post-Migration Steps)

Because transport barriers do not exist in Mule 4, Session properties are no longer needed and have been removed in Mule 4. After running the Mule Migration Assistant
(MMA) on a Mule app that uses Session properties, you should store data in variables (`vars`).
...
visibility into the app or its endpoints. So MMA cannot complete the
