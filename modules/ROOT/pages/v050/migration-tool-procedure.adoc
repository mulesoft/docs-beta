...
use of MMA, as well as post-migration tasks, on a sample Mule 3 application.
...
----
$ java -jar mule-migration-tool-runner-0.5.0.jar \
...
`my-migrated-project`) in which to place the migrated must _not_ exist. MMA
...
Mule Configuration XML files for your project that MMA outputs.
+
...
example, `/path/to/my/destination-folder`. MMA will create the
...
Whenever MMA adds an entry to the report (ERROR, WARNING, or INO), the
...
. Run MMA following instructions in <<run_tool>>.
