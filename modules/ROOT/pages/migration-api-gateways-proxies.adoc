...
The `src` directory contains directories `main` and `test`. Inside `main`, the `mule` directory
contains the proxy XML file. At the same level as the `mule` directory, MMA might create a
`resources` directory that contains DataWeave files or other files that the migrated proxy needs.
Note that configuration file defined in the proxy XML must be present in the `resources` directory
...
file. In this file, the `artifactId` for the proxy will be the name of the
