...
In Mule 3, the inbound and outbound endpoints for some transports had configurations 
that, in Mule 4, have been moved to the connector. The `http` and 
`vm` transports are examples. When the connector is declared in the
domain, the migration tool run for the domain has no visibility into the 
app and its endpoints. So the tool cannot complete the configuration of the connector, 
and the configuration must be completed manually after all the apps in the domain have been 
migrated.
...
when the connector is in a domain, it is necessary to migrate the Mule 4 configuration 
manually. Review the documentation of the equivalent Mule 3 and 
