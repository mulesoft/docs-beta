...
On Mule 3, the inbound/outbound endpoins for some transports had configuration 
that, in Mule 4, has been moved up to the connector. For instance, `http` and 
`vm` transports are in this situation. When the connector is declared in the
domain, since the migration tool run for the domain has no visibility of the 
apps and its endpoins, it cannot complete the configuration of the connector 
and it must be done manually after all the apps of the domain have been 
migrated.
...
In the above example, in Mule 3 the `port` is configured on the endpoint, so 
when the connector is in a domain, in Mule 4 that configuration has to be 
migrated manually. Review the documentation of the equivalent Mule 3 and 
Mule 4 connectors for details on how a specific configuration might be 
migrated.